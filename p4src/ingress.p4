    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    // 15-bit index (32768 slots, ~512 KB counter SRAM == same footprint as the
    // 17-bit packet register, which fits). 16-bit/65536 did NOT fit (stage 10
    // SRAM). Expected fold collisions ~P^2/65536 (≈3.8 at P=500, ≈15 at P=1000),
    // so per-flow byte parity stays clean to ~hundreds of simultaneous flows.
    // analyze_bytes.py must fold the 17-bit printed bucket with --counter-bits 15.
    Counter<bit<64>, bit<15>>(32768, CounterType_t.PACKETS_AND_BYTES) quic_flow_bytes;
    ...
    apply {
        ...
        if (hdr.quic_long.isValid() || hdr.quic_short.isValid()) {
            quic_flow_bytes.count((bit<15>)meta.flow_id);
